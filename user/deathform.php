...
if (strlen($_SESSION['obcsuid']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        // Check if the same date of death and father name exists in tbldeath
        $checkQuery = "SELECT * FROM tbldeath WHERE DateofDeath=:dod AND NameofFather=:fathername";
        $checkStmt = $dbh->prepare($checkQuery);
        $checkStmt->bindParam(':dod', $dod, PDO::PARAM_STR);
        $checkStmt->bindParam(':fathername', $fathername, PDO::PARAM_STR);
        $checkStmt->execute();

        if ($checkStmt->rowCount() == 0) {
            // Insert data into tbldeath
            $insertQuery = "INSERT INTO tbldeath (UserID, ApplicationID, DateofDeath, Gender, FullName, PlaceofBirth, NameOfMother, NameofFather, PermanentAdd, PostalAdd, MobileNumber, Email)
                            VALUES (:uid, :appnumber, :dod, :gender, :fname, :pob, :mname, :fathername, :padd, :postaladd, :mobnumber, :email)";
            $insertStmt = $dbh->prepare($insertQuery);
            $insertStmt->bindParam(':uid', $uid, PDO::PARAM_STR);
            $insertStmt->bindParam(':appnumber', $appnumber, PDO::PARAM_STR);
            $insertStmt->bindParam(':dod', $dod, PDO::PARAM_STR);
            $insertStmt->bindParam(':gender', $gender, PDO::PARAM_STR);
            $insertStmt->bindParam(':fname', $fname, PDO::PARAM_STR);
            $insertStmt->bindParam(':pob', $pob, PDO::PARAM_STR);
            $insertStmt->bindParam(':mname', $mname, PDO::PARAM_STR);
            $insertStmt->bindParam(':fathername', $fathername, PDO::PARAM_STR);
            $insertStmt->bindParam(':padd', $padd, PDO::PARAM_STR);
            $insertStmt->bindParam(':postaladd', $postaladd, PDO::PARAM_STR);
            $insertStmt->bindParam(':mobnumber', $mobnumber, PDO::PARAM_STR);
            ...
            if ($lastInsertId > 0) {
                ...
                echo "<script>window.location.href ='managedeath.php'</script>";
            } else {
                echo '<script>alert("Something Went Wrong. Please try again")</script>';
            }
        } else {
            echo "<script>alert('Date of Death and Father Name already exist. Please try again');</script>";
        }
    }
}
?>
...
<head>
    ...
    <title>Death Certificate Form | Online Death Certificate System</title>
                                                        ...
                                                        <div class="form-group-inner">
                                                            <div class="row">
                                                                ...
                                                                <div class="col-lg-9">
                                                                    ...
                                                                    <input type="date" class="form-control" name="dod" value="" required="true" />
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="form-group-inner">
